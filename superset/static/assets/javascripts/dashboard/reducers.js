...
import { ADD_SLICES } from './actions';

export function getInitialState(bootstrapData, drilldown = false) {
  const { user_id, datasources, common, editMode, user_name } = bootstrapData;
  ...
  try {
    ...
    const dataJson = key && sessionStorage.getItem(key);
    const data = JSON.parse(dataJson);

    extra_where = data.extra_where;
  } catch (e) {}
  ...
  if (!url.hid || !url.hid_index || !history.length) {
    dashboard.slices.forEach((slice) => {
      const chartKey = drilldown ? `slice_${slice.slice_id}_drilldown` : `slice_${slice.slice_id}`;
      initCharts[chartKey] = {
        ...chart,
        chartKey,
    ...
    if (existingDashboardState) {
      sessionStorage.setItem(
        url.hid,
        JSON.stringify(history.slice(0, url.hid_index))
      );
    }
  }
  ...
  return {
    ...
    dashboard: {
      ...
      dataForAsyncRender: [],
      checkAsyncModeForAddedSlice: false,
    },
  };
}
...
export const dashboard = function(state = {}, action) {
  const actionHandlers = {
    [actions.CLOSE_DRILLDOWN_TO_DASHBOARD]() {
      const clearSelectedPoints = action.selectedItem
    ...
    [actions.DRILLDOWN_TO_DASHBOARD]() {
      return {
        ...state,
        drilldown: true,
        drilldownData: action.drilldown,
        left: action.left,
        top: action.top,
    ...
    [actions.SET_DATA_FOR_ASYNC_RENDER]() {
      return {
        ...state,
    ...
    [actions.REMOVE_EL_FOR_ASYNC_RENDER]() {
      return {
        ...state,
    ...
    [actions.ADD_SLICES_SUCCESS]() {
      const { slices: addedSlices, datasources: addedDatasources } = action;
      const { datasources, dashboard: stateDashboard } = state;
      const { slices, slices_perms, layout, position_json } =
        stateDashboard || {};
      ...
      const lastRowId = Math.max(
        ...
        Math.max.apply(
          null,
          stateDashboard.position_json.map((pos) => pos.row + pos.size_y)
        )
      );

      let sliceCounter = layout.length;

      const isAsyncMode = addedSlices[0]?.form_data?.allow_run_async || false;

      addedSlices.forEach((sliceData) => {
        const { form_data: formData } = sliceData;
        const sliceId = formData.slice_id;
        let pos = stateDashboard.posDict[sliceId];
        if (!pos) {
          // append new slices to dashboard bottom, 3 slices per row
          pos = {
            col: (sliceCounter % 3) * 16 + 1,
            row: lastRowId + Math.floor(sliceCounter / 3) * 16,
            size_x: 16,
            size_y: 16,
          };
          stateDashboard.posDict[sliceId] = pos;
          sliceCounter++;
        }

        position_json.push({ ...pos });
        layout.push({
          i: String(sliceId),
          x: pos.col - 1,
          y: pos.row,
        ...
        const { url: sliceUrl } = getExploreUrlAndPayload({
          formData,
          endpointType: 'json',
          force: true,
        });
        slices.push({
          ...
          formData: applyDefaultFormData(formData),
          form_data: formData,
          slice_id: sliceId,
      ...
      return {
        ...
        isLoading: false,
        unsavedChanges: !!addedSlices.length,
        checkAsyncModeForAddedSlice: isAsyncMode,
      };
    },
    [actions.REMOVE_SLICE]() {
      ...
      const newPositionJson = state.dashboard.position_json.filter(
        (jsonPos) => jsonPos.slice_id !== key
      );
    ...
    [actions.ADD_FILTER]() {
      ...
      const { sliceId, col, vals, op, merge, refresh } = action;
      const filterKeys = [
        '__from',
        '__to',
        '__time_col',
      ...
      if (
        ...
        const operator = (col && op[col] && op) ? op[col].value : 'in';
        const newFilter = { col, op: operator, val: vals, source: 'filter_box' };
        ...
        if (vals && !vals.length) {
          ...
          if (existingFilter) {
            ...
            filter.col === col ? { ...filter, op: operator, val: vals } : filter
          );
          } else {
    ...
    [chartActions.SAVE_SLICE_STATE]() {
      const drilldownData =
        (state.drilldownData &&
          state.drilldownData.find((data) =>
            data.dashboard.dashboard.slices.find(
              (slice) => slice.slice_id === action.sliceId
            )
          )) ||
        [];
      const slice = action.drilldown
        ? drilldownData.dashboard &&
          drilldownData.dashboard.dashboard.slices.find(
            (item) => item.slice_id === action.sliceId
          )
        : state.dashboard.slices.find(
            (item) => item.slice_id === action.sliceId
          );
      const prevFormData = {
      ...
      const setPrevFormData = (item) =>
        item === slice
          ...
          : item;
      const newSlices = state.dashboard.slices.map(setPrevFormData);
      const newDrilldownSlices =
        drilldownData.dashboard &&
        drilldownData.dashboard.dashboard.slices.map(setPrevFormData);
      drilldownData.dashboard &&
        (drilldownData.dashboard.dashboard.slices = newDrilldownSlices);
    ...
    [chartActions.REVERT_SLICE_STATE]() {
      const drilldownData =
        ...
        [];
      const slice = action.drilldownFromPopUp
        ? drilldownData.dashboard &&
          drilldownData.dashboard.dashboard.slices.find(
            (item) => item.slice_id === action.sliceId
          )
      ...
      const setNewSlices = (item) =>
        item === slice
          ...
          : item;
      const newSlices = state.dashboard.slices.map(setNewSlices);
      const newDrilldownSlices =
        drilldownData.dashboard &&
        drilldownData.dashboard.dashboard.slices.map(setNewSlices);
      drilldownData.dashboard &&
        (drilldownData.dashboard.dashboard.slices = newDrilldownSlices);
      return { ...state, dashboard: { ...state.dashboard, slices: newSlices } };
    },
    [chartActions.DRILLDOWN_TO_SLICE]() {
      const drilldownData =
        (state.drilldownData &&
          state.drilldownData.find((data) =>
            data.dashboard.dashboard.slices.find(
              (slice) => `slice_${slice.slice_id}_drilldown` === action.key
            )
          )) ||
        [];
      ...
      const slice = action.drilldownFromPopUp
        ? drilldownData.dashboard &&
          drilldownData.dashboard.dashboard.slices.find(
            (item) => `slice_${item.slice_id}_drilldown` === action.key
          )
        : state.dashboard.slices.find(
