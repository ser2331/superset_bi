...
class ConnectorRegistry(object):
    ...
    def get_eager_datasource(cls, session, datasource_type, datasource_id):
        ...
        # в модели SqlaTable атрибуты `columns` и `metrics` являются
        # property и содержат в себя несколько других запросов,
        # поэтому в `subqueryload` не получиться их подтянуть
        if datasource_class.__name__ == 'SqlaTable':
            options_args = tuple()
        else:
            options_args = (
