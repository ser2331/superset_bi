...
const propTypes = {
  ...
  exploreActions: PropTypes.object,
...
class Chart extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      height: this.props.height || this.container?.el.offsetHeight,
      drilldownIsOpen: false
    };
    // these properties are used by visualizations
    this.drilldownData = props.drilldownData
    ...
    this.columns = this.props.datasource.columns || [];
    this.metrics = this.props.datasource.metrics || [];
  }
  ...
  componentWillReceiveProps(nextProps) {
    this.drilldownData = nextProps.drilldownData
    ...
    this.prevFormData = nextProps.prevFormData;
  }
  ...
  addFilter(col, vals, op, merge = true, refresh = true) {
    this.props.addFilter(col, vals, op, merge, refresh);
  }
  ...
  getLegendHeight = (id) => {
    const legendId = this.getLegend(id);
  ...
  width() {
    return this.props.width || this.container?.el?.offsetWidth || 0;
  }
  ...
  height() {
    const legend = this.getLegendHeight(this.containerId);
    return (
      (this.props.height || this.container.el.offsetHeight) + legend - (this.paginationHeight || 0)
    );
  }

  appendPaginationHeight(paginationHeight) {
    if (paginationHeight !== this.paginationHeight) {
  ...
  renderViz() {
    ...
    try {
      ...
      // payload = qr = queryResponse => redux charts.slice_64.queryResponse
      // slice - то, что положили в this из props (ChartContainer and ExploreChartPanel - ExploreViewContainer)
      console.log({slice: this, payload: qr})
      this.props.vizType === 'bubble_map'
        ? viz(
            this.props.openDrilldown,
            this.props.closeDrilldown,
            this.props.clearDrilldownCharts,
            this,
            qr,
            this.props.setControlValue,
            this.props.metadata
          )
        : viz(this, qr, this.props.setControlValue, this.props.metadata);
