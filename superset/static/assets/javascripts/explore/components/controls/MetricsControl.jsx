...
import { D3_FORMAT_OPTIONS } from '../../stores/controls';
...
export default class MetricsControl extends React.PureComponent {
  ...
  componentDidMount() {
    setTimeout(() => {
      ...
      metricControl.isRenderedRecently = false;
      metricControl.value = typeof this.props?.value === 'string' ? this.props?.value : this.props?.value?.filter(item => typeof item === 'string');
    }, 3000);
  }

  componentWillReceiveProps(nextProps) {
    if (this.props.vizType === "table" || this.props.vizType === "pivot_table" ) {
      const newValue = nextProps?.value?.filter(item => typeof item === 'string')
      if (Array.isArray(newValue) && newValue.length < metricControl.value.length) {
        metricControl.value = newValue
      }
    }
    if (
  ...
  onMetricEdit(changedMetric) {
    let newValue
    if (typeof changedMetric === 'string') {
      newValue = changedMetric
    } else {
      newValue = this.state.value.map((value) => {
  ...
  onChange(opts) {
    ...
    let optionValues = transformedOpts.map((option) => {
      if (option.metric_name) {
        ...
        const commonMetricFormatValue = this.props.commonMetricFormatValue

        const existedMetricFormat = D3_FORMAT_OPTIONS.find((option) => option[0] === commonMetricFormatValue)
        const transformedMetricFormat = existedMetricFormat ? {value : existedMetricFormat[0], label: existedMetricFormat[1]} : undefined

        const commonMetricFormat = commonMetricFormatValue ? (transformedMetricFormat || {value: commonMetricFormatValue, label: commonMetricFormatValue}) : undefined
  ...
  optionsForSelect(props) {
    ...
    return options.map((option) => {
      if (option.metric_name) {
        return { ...option, optionName: option.metric_name, value: option.verbose_name || option.metric_name };
      } else if (option.column_name) {
        return { ...option, optionName: '_col_' + option.column_name, value: option.verbose_name || option.column_name };
      } else if (option.aggregate_name) {
        return { ...option, optionName: '_aggregate_' + option.aggregate_name, value: option.verbose_name || option.aggregate_name };
      }
      // notify.error(t('provided invalid option to MetricsControl, %s', option));
      console.error(t('provided invalid option to MetricsControl, %s', option));
      return null;
    });
  }
  ...
  selectFilterOption(option, filterValue) {
    let userInput = filterValue;
    if (this.state.aggregateInInput) {
      let endIndex = userInput.length;
      if (filterValue.endsWith(')')) {
        endIndex = filterValue.length - 1;
      }
      userInput = filterValue.substring(filterValue.indexOf('(') + 1, endIndex);
    }

    if (!userInput) return true;
    if (option.verbose_name && option.verbose_name.toLowerCase().includes(userInput.toLowerCase())) {
      return true;
    }

    if (this.state.aggregateInInput) {
      return option.column_name &&
        (option.column_name.toLowerCase().indexOf(userInput.toLowerCase()) >= 0);
    }
    return option.optionName &&
      (!option.metric_name || !this.isAutoGeneratedMetric(option) || option.metric_name && this.isAutoGeneratedMetric(option)) &&
      (option.optionName.toLowerCase().indexOf(userInput.toLowerCase()) >= 0);
  }
  ...
  shouldComponentUpdate(nextProps, nextState) {
    const allPropsMatched = Object.keys(nextProps)
        .every(key => _.isEqual(nextProps[key], this.props[key]));
    ...
    return (
      <div className="metrics-select">
        ...
        <OnPasteSelect
          ...
          options={!this.state.selectedGroup ? this.state.options : this.state.options.filter((option) => {
            const groupIndex = this.state.selectedGroup.value;
            const optionName = option.column_name || option.metric_name;
            return this.props.groups[groupIndex].columns.includes(optionName);
          })}
