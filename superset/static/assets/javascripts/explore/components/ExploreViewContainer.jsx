...
const propTypes = {
  ...
  can_config: PropTypes.bool,
  can_edit: PropTypes.bool,
  can_favstar: PropTypes.bool,
...
class ExploreViewContainer extends React.Component {
  ...
  componentWillReceiveProps(np) {
    ...
    if (np.form_data.page_length !== this.props.form_data.page_length) {
    ...
    if (this.hasDisplayControlChanged(changedControlKeys, np.controls)) {
      this.props.actions.updateQueryFormData(
        getFormDataFromControls(np.controls),
    ...
    if (this.hasQueryControlChanged(changedControlKeys, np.controls) || this.hasColumnFormatsChanged(this.props.datasource, np.datasource)) {
      this.setState({ chartIsStale: true, refreshOverlayVisible: true });
    }
  }

  handleAsyncReqStarted() {
    this.setState({ asyncReqStarted: !this.state.asyncReqStarted });
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const isAllowAsync = this.props.slice?.allow_run_async || false;
    const isAllLoaded =
      !!data.length && data.every(({ isLoading }) => isLoading === false);

    if (isAllowAsync && isAllLoaded && !this.state.asyncReqStarted) {
      fetchAsyncRunQueries(
        this.props.dataForAsyncRender,
        this.handleAsyncReqStarted,
        this.props.actions.setAsyncResponseToRender,
        this.props.actions.removeElForAsyncRenderExplore
      );
    }
  }
  ...
  hasColumnFormatsChanged(prevColumnFormats, nextColumnFormats) {
    return Object.keys(prevColumnFormats).some(
      formatName => prevColumnFormats[formatName] !== nextColumnFormats[formatName]
    );
  }

  triggerQueryIfNeeded() {
    if (this.props.chart.triggerQuery && !this.hasErrors()) {
      // runQuery on button Run Query
  ...
  renderErrorMessage() {
    ...
    if (errors.length > 0) {
      return <div style={{ textAlign: 'left' }}>{errors}</div>;
    }
  }
  ...
  render() {
    ...
    const { can_config: canConfig } = this.props;
    // to send column_formats on server for table and pivot_table
    const formDataToSave = this.props.vizType === "table" || this.props.vizType === "pivot_table" ? {...this.props.form_data, column_format: this.props.datasource?.column_format} : this.props.form_data
...
function mapStateToProps(state) {
  ...
  return {
    ...
    can_edit: !!explore.can_edit,
    can_favstar: !!explore.can_favstar,
    ...
    form_data: {
      ...form_data,
      ...(explore.slice?.allow_run_async && {
        allow_run_async: explore.slice?.allow_run_async,
      }),
    ...
    impressionId,
    dataForAsyncRender: explore.dataForAsyncRender,
  };
}
