...
export default class DateFilterControl extends React.Component {
  ...
  componentDidMount() {
    this.initValues(this.props.value)
  }

  componentWillReceiveProps(nextProps) {
    this.initValues(nextProps.value)
  }
  ...
  initValues(value) {
    ...
    if (words.length >= 3 && RELATIVE_TIME_OPTIONS.indexOf(words[2]) >= 0) {
      newState.num = words[0];
      newState.grain = words[1];
      newState.rel = words[2];
      newState.type = 'rel';
      ...
      <Popover id="filter-popover">
        <div style={{ width: '250px' }}>
          ...
          >
            <InputGroup bsSize="small">
              ...
              <Datetime
                ...
                defaultValue={
                  moment(this.state.dttm).isValid() ?
                    moment(this.state.dttm).format("YYYY-MM-DD HH:mm:ss") :
                    this.state.dttm
                }
            ...
            <div className="clearfix">
              ...
              <div style={{ width: '95px' }} className="input-inline">
                <Select
                  ...
                  clearable={false}
                  options={TIME_GRAIN_OPTIONS.map(s => ({
                    label: tn(s.substring(0, s.length - 1), s, 5),
                    value: s,
                  }))}
              ...
              <div style={{ width: '95px' }} className="input-inline">
                <Select
                  ...
                  onFocus={this.setType.bind(this, 'rel')}
                  clearable={false}
                  options={RELATIVE_TIME_OPTIONS.map(s => ({
                    label: t(s),
                    value: s,
                  }))}
          ...
          {/*<PopoverSection
            ...
            info={t('Superset supports smart date parsing. Strings like `last sunday` or `last october` can be used.')}
          >
            <FormControl
              onFocus={this.setType.bind(this, 'free')}
              value={t(this.state.free)}
  ...
  render() {
    const value = dehumanizeDate(this.props.value);
    return (
      <div>
