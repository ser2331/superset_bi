...
export const SAVE_QUERY_CLEAR_ERROR = 'SAVE_QUERY_CLEAR_ERROR';
...
export function saveQuery(query) {
  return function (dispatch) {
    ...
    $.ajax({
      ...
      error: (err) => {
        notify.error(t('Your query could not be saved'));
        let saveError = null;
        if (err.responseJSON && err.responseJSON.message) {
          saveError = {
            message: err.responseJSON.message,
            details: err.responseJSON.details,
          };
        }
        dispatch({ type: SAVE_QUERY, saveError });
      },
      dataType: 'json',
    });
  };
...
export function clearError() {
  return { type: SAVE_QUERY_CLEAR_ERROR };
}

export function querySuccess(query, results, pageOffset) {
  return { type: QUERY_SUCCESS, query, results, pageOffset };
}
...
export function runQuery(query, pageOffset) {
  return function (dispatch, getState) {
    const { queryPageSize } = getState();
    ...
    const sqlJsonRequest = {
      ...
      limit: queryPageSize,
      pageOffset: pageOffset || 0, // page change or query button
    };
    ...
    $.ajax({
      ...
      success(results) {
        if (!query.runAsync) {
          dispatch(querySuccess(query, results, pageOffset));
        }
      },
