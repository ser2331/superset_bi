...
class BaseDatasource(AuditMixinNullable, ImportMixin):
    ...
    def column_formats(self):
        return {
            ...
            if m.d3format is not None
        }

    def column_formats_func(self, session=None):
        return {
            m.metric_name: m.d3format
            for m in self.get_metrics_filter(session=session).all()
            if m.d3format is not None
        }

    def add_missing_metrics(self, metrics, is_meta_update=False):
        ...
        for metric in metrics:
            if metric.metric_name not in exisiting_metrics:
                ...
                if is_meta_update:
                    metric.is_meta_update = True
                missing_metrics.append(metric)
        return missing_metrics

    @property
    def metrics_combo(self):
        return self.metrics_combo_func(self.metrics)

    @staticmethod
    def metrics_combo_func(metrics):
        return sorted(
            [
    ...
    def data(self, session=None):
        ...
        order_by_choices = []
        if session:
            metrics = self.get_metrics_filter(session=session)
            columns = self.get_columns_filter(session=session)
            sorted_columns = sorted([c.column_name for c in columns])
            groupby_column_names = sorted([c.column_name for c in columns if c.groupby])
            filterable_column_names = sorted([c.column_name for c in columns if c.filterable])
            metrics_combo = self.metrics_combo_func(metrics)
            column_formats = self.column_formats_func(session=session)

        else:
            metrics, columns, sorted_columns = self.metrics, self.columns, self.column_names
            groupby_column_names = self.groupby_column_names
            filterable_column_names = self.filterable_column_names
            metrics_combo = self.metrics_combo
            column_formats = self.column_formats

        for s in sorted(sorted_columns):
            order_by_choices.append((json.dumps([s, True]), s + _(' [asc]')))
            order_by_choices.append((json.dumps([s, False]), s + _(' [desc]')))

        verbose_map = {'__timestamp': __('Time')}
        verbose_map.update({
        ...
        return {
            'all_cols': utils.choicify(sorted_columns),
            'column_formats': column_formats,
            'database': self.database.data,  # pylint: disable=no-member
            'edit_url': self.url,
            'filter_select': self.filter_select_enabled,
            'filterable_cols': utils.choicify(filterable_column_names),
            'gb_cols': utils.choicify(groupby_column_names),
            'id': self.id,
            'metrics_combo': metrics_combo,
            'name': self.name,
            'order_by_choices': order_by_choices,
            'type': self.type,
            'metrics': [o.data for o in metrics],
            'columns': [o.data for o in columns],
