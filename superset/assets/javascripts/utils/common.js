...
export function dehumanizeDate(value) {
    ...
    const containsFromNow = RELATIVE_TIME_OPTIONS.includes(`${words[2]} ${words[3]}`);

    if (words.length >= 3 && (containsAgo || containsFromNow)) {
        const number = Number.parseInt(words[0], 10);
        const grainSingleForm = words[1].endsWith('s') ? words[1].substring(0, words[1].length - 1) : words[1];
