    ...
    if (Math.round(queryDuration.asMilliseconds()) > timeout * 1000) {
      advise = (
        <Alert bsStyle="warning">
          ...
          {t('following this flow will most likely lead to your query timing out. We recommend your summarize your data further before following that flow.')}
          {t('If activated you can use the ')}
          <strong>CREATE TABLE AS</strong>
          {t('to store a summarized data set that you can then explore.')}
        </Alert>);
    }
    return advise;
  }
        ...
        <Modal show={this.props.show} onHide={this.props.onHide}>
          ...
          <Modal.Body>
            ...
            <Table
              ...
              columns={[
                { key: 'column', label: t('Column') },
                { key: 'is_dimension', label: t('Is dimension?') },
                { key: 'is_date', label: t('Is date?') },
                { key: 'agg_func', label: t('Aggregate function') },
              ]}
              data={tableData}
            />
            <Button
