...
class BaseEngineSpec(object):
    ...
    inner_joins = True
    row_number_column = None # Используется в MssqlEngineSpec
    ...
    sqla_aggregations_with_cumulative_total = {
        'COUNT_DISTINCT': lambda column_name: sqla.func.SUM(sqla.func.COUNT(sqla.distinct(column_name))),
        'SUM': lambda column_name: sqla.func.SUM(sqla.func.SUM(column_name)),
        'MIN': lambda column_name: sqla.func.MIN(sqla.func.MIN(column_name)),
        'MAX': lambda column_name: sqla.func.MAX(sqla.func.MAX(column_name)),
    }
    ...
    def validate_column_names(cls, df):
        """Валидирует названия столбцов"""
        regex = re.compile(r'(^[A-Za-z_]+[A-Za-z0-9_]*$)')
        result = [regex.fullmatch(x) for x in df.columns]
        no_dups = len(set((c.lower() for c in df.columns))) == len(df.columns)
        result.append(no_dups)
        if not all(result):
            raise Exception(
                _('Invalid column name (one or more) in csv file.')
                + '<br/>'
                + '<br/>'
                + _('- Column names should contain only numbers, latin letters and underscore: [A-Z] [a-z] [0-9] [ _ ].')
                + '<br/>'
                + _('- Column names should not begin with number: \"12_pokazatyel\".')
                + '<br/>'
                + _('- Columns names should not be repeated in one file: \"Kolonka\", \"kolonka\", \"kolonka_123\".')
                + '<br/>'
                + '<br/>'
                + _('Examples:')
                + '<br/>'
                + '<br/>'
                + _('- Valid names: \"Kolonka\", \"stolbec\", \"kolonka_123\".')
                + '<br/>'
                + _('- Invalid names: \"123kolonka\", \"Столбец #57\".')
            )

    @classmethod
    def csv_to_df(cls, **kwargs):
    ...
    @staticmethod
    def create_table_from_csv(form, table, encoding=None):
    ...
    @classmethod
    def wrap_sql_limit(self, sql, database, limit=1000, offset=0):
        qry = (
            select('*')
                .select_from(
                TextAsFrom(text(sql), ['*'])
        ...
        compiled_qry = database.compile_sqla_query(qry)

        logging.info("Wrapped offset and limit query %s" % compiled_qry)
...
class MssqlEngineSpec(BaseEngineSpec):
    ...
    row_number_column = '__row_number__' # Должно отличаться от существующих столбцов в таблице
    ...
    @classmethod
    def wrap_sql_limit(cls, sql, database, limit=1000, offset=0):
        # Т.к. MSSQL не поддерживает OFFSET, был создан запрос с применением
        # конструкции ROW_NUMBER() OVER - однако средствами sqlalchemy эту конструкцию создать не удалось.
        # Поэтому здесь "чистый" SQL.
        if not sql.upper().startswith('SELECT'):
            return sql

        # Если в запросе был передан SELECT TOP <N> то необходимо выявить это и сформировать корректный запрос.
        with_top = re.match(r'(^\s?SELECT\s+TOP)', sql.upper())

        if with_top:
            pre_sql = re.sub(r'(^\s?SELECT\s+TOP\s+)', '', sql, flags=re.IGNORECASE)
            top_rows = int(re.match(r'(^\s?[0-9]*)', pre_sql).group(0).strip())
            sql = re.sub(r'(^\s?[0-9]*)', '', pre_sql)
        else:
            sql = re.sub(r'(^\s?SELECT\s+)', '', sql, flags=re.IGNORECASE)
            top_rows = None


        compiled_qry = """WITH OrderedTable AS
            (
                SELECT {top} ROW_NUMBER() OVER (ORDER BY (SELECT 1)) AS "{row_number_column}", {inner_query}
            )
            SELECT *
            FROM OrderedTable
            WHERE {row_number_column} BETWEEN {offset} + 1 AND {limit} + {offset}""".format(
            inner_query=sql,
            offset=offset,
            limit=limit,
            row_number_column = cls.row_number_column,
            top='TOP {}'.format(top_rows) if with_top else ''
        )
        logging.info("Wrapped offset and limit query %s" % compiled_qry)
...
class ClickHouseEngineSpec(BaseEngineSpec):
    ...
    sqla_aggregations = {
        'COUNT_DISTINCT': lambda column_name: literal_column('uniqExact({})'.format(column_name)).label(
            'count_distinct({})'.format(column_name)),
        'COUNT': lambda column_name: literal_column('count({})'.format(column_name)).label(
    ...
    sqla_aggregations_with_cumulative_total = {
        'COUNT_DISTINCT': (
            lambda column_name: literal_column('uniqExactArray(groupUniqArray({}))'.format(column_name))
                .label('count_distinct({})'.format(column_name))
        ),
        'SUM': lambda column_name: sqla.func.SUM(sqla.func.SUM(column_name)),
        'MIN': lambda column_name: sqla.func.MIN(sqla.func.MIN(column_name)),
        'MAX': lambda column_name: sqla.func.MAX(sqla.func.MAX(column_name)),
