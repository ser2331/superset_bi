...
export const controls = {
    ...
    mapStateToProps: (state) => {
      ...
      if (datasource && datasource.metric_groups) {
          ...
          Object.keys(datasource.metric_groups).forEach((key) => {
              ...
              const knownFolderKey = Object.keys(groups).find(item => groups[item].label === label);
              if (knownFolderKey) {
                  groups[knownFolderKey].columns = [ ...groups[knownFolderKey].columns, ...metrics];
              } else {
          ...
          Object.keys(datasource.column_groups).forEach((key) => {
              ...
              const knownFolderKey = Object.keys(groups).find(item => groups[item].label === label);
              if (knownFolderKey) {
                  groups[knownFolderKey].columns = [ ...groups[knownFolderKey].columns, ...columns];
              } else {
  ...
  percent_metrics: {
    ...
    filterOptions: (options, filter, currentValues) => !filter ? options : options
      .filter(o => (o.verbose_name || o.metric_name).includes(filter) && (!currentValues || !currentValues.find(c => c.metric_name === o.metric_name))),
  ...
  order_by_cols: {
    ...
    mapStateToProps: (state) => {
      ...
      const getVerboseName = (colNameRaw) => {
        // colNameRaw = "["status_testa", true]"
        const valueParsed = JSON.parse(colNameRaw);
        const colName = valueParsed[0];
        const asc = valueParsed[1];
        const column = state.datasource.columns.find(c => c.column_name === colName);
        return column && column.verbose_name ? `${column.verbose_name} [${t(asc ? 'ASC short' : 'DESC short')}]` : '';
      };
  ...
  timeseries_limit_metric: { // TODO: Почему ORDER BY так странно называется?!
    type: 'SelectControl',
    label: t('Sort By'),
    default: null,
    description: t('Metric used to define the top series'),
    mapStateToProps: (state) => {
        const source = (state.controls && state.controls.groupby && state.controls.groupby.value || []) ||
          state.form_data && state.form_data.groupby || []; // При первой загрузке контролов еще нет
        const groupby = source.map((item) => {
            const matchingColumn = state.datasource.columns.find(column => column.column_name === item);
            return matchingColumn && matchingColumn.verbose_name
                ? [ item, matchingColumn.verbose_name ]
                : [ item, item ];
        });
        return {
            choices: (state.datasource) ? [ ...state.datasource.metrics_combo, ...groupby ] : [],
        };
    },
  },
  ...
  point_radius: {
    ...
    mapStateToProps: state => ({
      choices: [].concat([['Auto', 'Автоматически']], state.datasource.all_cols),
    }),
  },
  ...
  disabled_hierarchy: {
    ...
    mapStateToProps: state => ({
      choices: (state.hierarchies)
        ? state.hierarchies.filter(h => h.table_id === state.datasource_id).map(h => [h.id, h.name])
        : [],
    }),
  },
