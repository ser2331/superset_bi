...
export default class TemplateParamsEditor extends React.Component {
  ...
  renderDoc() {
    return (
      <p>
        {t('Assign a set of parameters as ')}<code>JSON</code> {t(' below')}
        ({t('example')}: <code>{'{"my_table": "foo"}'}</code>),
        {t('and they become available')}
        {t('in your SQL ')}({t('example')}: <code>SELECT * FROM {'{{ my_table }}'} </code>)
        {t('by using')}
        <a
