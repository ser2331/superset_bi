...
import { formatDate, UTC, makeFormatDateExt } from "./dates"
...
export function d3FormatPreset(format, timeGrain) {
  // like d3.format, but with support for presets like 'smart_date'
  if (format === "smart_date") {
    if (timeGrain) {
      return makeFormatDateExt(timeGrain)
    }
    return formatDate
  }
  if (format) {
...
export const d3TimeFormatPreset = function (format, utc_offset, timeGrain) {
  ...
  return function (dttm) {
    // dttm = utc_offset? (dttm + (utc_offset * 3600 * 1000)) : dttm;
    let d = new Date(dttm);
    let offset
    if(typeof dttm === "number"){
      offset = new Date(dttm).getTimezoneOffset()
      d = new Date(dttm);
     } 
    
    if(typeof dttm === "string"){
     offset = new Date(Date.parse(dttm)).getTimezoneOffset();
     d = new Date(dttm);
    } 

    if(typeof dttm === "object"){
      d = dttm;
    }
    if (isNaN(d) || dttm === null) {
      return String(dttm)
    }
    return f(d);
  };
...
export function formatSelectOptionsForRange(start, end) {
  ...
  for (let i = start; i <= end; i++) {
    options.push([i, t(i.toString())])
  }
  return options
}

export function formatSelectOptions(options) {
  return options.map((opt) => [opt, t(opt.toString())])
}

export const getZoomSelectOptions = () =>
