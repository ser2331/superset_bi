...
import { bigNumberDDVisualization } from "./helpers/ddVisualization/bigNumberDD";
...
function bigNumberVis(slice, payload) {
  ...
  const menu = () => {
    ...
    return hierarcyManager.getUrlDrilldowns(fd.metric, []);
  };

  const f = d3FormatPreset(fd.y_axis_format, fd.time_grain_sqla);
  ...
  g.append("g")
    ...
    .text(f(v))
    // .style('font-size', d3.min([height, width]) / 6)
    .style("font-size", width / 8)
  ...
  if(json.subheader !== null) {
    const bigNumber = document.querySelector(`#${slice.containerId} .digits text.big`);
    const arrtY = bigNumber?.getAttribute("y");
    const bignumberHeight = bigNumber?.getBoundingClientRect().height;
    const yForPostFix = Math.round(arrtY) + Math.round(bignumberHeight / 2) + Math.round(width / 20);
    g.append("text")
      .attr("x", width / 2)
      // .attr('y', (height / 40) * 30)
      .attr("y", yForPostFix)
      ...
      // .style('font-size', d3.min([height, width]) / 20)
  ...
  //визуализация DD
  bigNumberDDVisualization(slice);
}
