...
class DashboardTests(SupersetTestCase):
    ...
    def test_save_dash(self, username='test_user'):
    ...
    def test_save_dash_with_filter(self, username='test_user'):
        self.login(username=username)
        dash = db.session.query(models.Dashboard).filter_by(
    ...
    def test_save_dash_with_dashboard_title(self, username='test_user'):
        self.login(username=username)
        dash = (
            db.session.query(models.Dashboard)
            .filter_by(slug='births')
    ...
    def test_copy_dash(self, username='test_user'):
        self.login(username=username)
        dash = db.session.query(models.Dashboard).filter_by(
            slug='births').first()
        positions = []
    ...
    def test_add_slices(self, username='test_user'):
        self.login(username=username)
        dash = db.session.query(models.Dashboard).filter_by(
            slug='births').first()
    ...
    def test_dashboard_with_created_by_can_be_accessed_by_public_users(self):
        ...
        dash = db.session.query(models.Dashboard).filter_by(
            slug='births').first()
        dash.owners = [security_manager.find_user('test_user')]
        dash.created_by = security_manager.find_user('test_user')
        db.session.merge(dash)
        db.session.commit()

        assert 'Births' not in self.get_resp('/superset/dashboard/births/')
